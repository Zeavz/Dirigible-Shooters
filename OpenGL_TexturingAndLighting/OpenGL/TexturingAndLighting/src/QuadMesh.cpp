...
bool QuadMesh::InitMesh(int meshSize, VECTOR3D origin, double meshLength, double meshWidth, VECTOR3D dir1, VECTOR3D dir2)
{
	...
	for (int j = 0; j < 14; j++) {
		...
		int h;
		din >> ax >> ay >> az >> h;
		tempBlob.cCordinates = VECTOR3D(ax,ay,az);
		tempBlob.height = h-5;
		tempBlob.width = 0.08;
		blob.push_back(tempBlob);
	}
	totalBlobs = 14;
	static float euler = 2.71828182845904523536;
	// loop through each vertice and update y, while looping through each permanent blob for each vertex
	for (int i = 0; i < numVertices + 1; i++)
	{
		for (int j = 0; j < totalBlobs; j++) {
			float r = sqrt((vertices[i].position.GetX() - blob[j].cCordinates.GetX())*(vertices[i].position.GetX() - blob[j].cCordinates.GetX()) + (vertices[i].position.GetZ() - blob[j].cCordinates.GetZ())*(vertices[i].position.GetZ() - blob[j].cCordinates.GetZ()));
			vertices[i].position.SetY(vertices[i].position.GetY() + (blob[j].height*(pow(euler, (-blob[j].width*(r*r))))));
		}
	}
...
void QuadMesh::DrawMesh(int meshSize, bool isItQuad)
{
	int currentQuad = 0;
	...
	for (int j = 0; j< meshSize; j++)
	{
		for (int k = 0; k< meshSize; k++)
		{
			...
			// for face normals only, we only need to calculate and use one of the normals
			// because any of the normals from above would be a normal of the face I chose to use
			// the first
			else {
				glNormal3f(quads[currentQuad].vertices[0]->normal.x,
					quads[currentQuad].vertices[0]->normal.y,
